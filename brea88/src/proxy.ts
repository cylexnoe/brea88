...
function isValidAdminSession(token: string | undefined): boolean {
  ...
  /*
   * Admin sessions do not expire based on time.
   *
   * The timestamp is still part of the signed token,
   * so the token cannot be modified without invalidating
   * the HMAC signature.
   */
...
export function proxy(request: NextRequest) {
   ...
   *
   * Admin session does NOT expire based on time.
   * It remains valid until the admin logs out
   * or the session secret is changed.
   */
  ...
  /*
   * Agent protected pages
   *
   * Agent sessions continue to expire after 30 days.
   */
  if (
    pathname === '/profile' ||
  ...
  if (
    ...
    const limit = rateLimit(
      getClientKey(request, 'public-inquiry'),
      12,
    );
