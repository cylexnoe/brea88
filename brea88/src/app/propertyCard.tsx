    ...
    <>
      ...
      {showInquiry && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-md" onClick={() => !inquirySubmitting && setShowInquiry(false)}>
        <div onClick={(event) => event.stopPropagation()} className="relative max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-[1.5rem] bg-white shadow-2xl">
          ...
          <div className="bg-[#071936] p-6 text-white sm:p-7"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#ead9b8]">BREA 88 REALTY</p><h2 className="mt-2 text-2xl font-black">Send an Inquiry</h2><p className="mt-1 line-clamp-2 text-sm text-white/70">{property.title}</p></div>
          {inquirySuccess ? <div className="p-7 text-center sm:p-9"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 text-emerald-600"><CheckCircle2 className="h-8 w-8" /></div><h3 className="mt-5 text-xl font-black text-slate-950">Inquiry Submitted</h3><p className="mt-2 text-sm leading-6 text-slate-500">Thank you. Your inquiry has been submitted successfully. Our team will get back to you soon.</p><button type="button" onClick={() => setShowInquiry(false)} className="mt-6 min-h-11 rounded-xl bg-[#071936] px-6 py-3 text-sm font-bold text-white transition hover:bg-slate-800">Done</button></div> : <form onSubmit={async (event) => {
            event.preventDefault();
            setInquirySubmitting(true); setInquiryError('');
            try {
              const response = await fetch('/api/inquiries', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ propertyId: property.id, ...inquiryForm, ...(property.agent?.slug ? { agentSlug: property.agent.slug } : {}) }) });
            ...
            <div className="rounded-xl border border-[#ead9b8] bg-[#faf7ef] p-3 text-xs leading-5 text-slate-600">Tell us how we can help. Your inquiry will be routed through the registered account associated with this property when an agent profile is available.</div>
