...
|--------------------------------------------------------------------------
|
| New accounts use scrypt with a unique random salt.
| The format is:
|
| scrypt$<salt>$<derived-key>
|
| This replaces the previous unsalted SHA-256 password storage.
|--------------------------------------------------------------------------
*/
...
export async function POST(request: Request) {
  try {
    ...
    if (fullName.length < 2) {
      return NextResponse.json(
        { success: false, message: 'Full name is too short.' },
        { status: 400 }
      );
    }
    ...
    // Public registration can never choose Broker or Admin.
    const role = 'Agent';
    ...
    return NextResponse.json(
      { success: false, message: 'Unable to create agent account.' },
      { status: 500 }
    );
  }
}
